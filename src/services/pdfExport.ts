...
export const exportToPdf = async (elementId: string, resumeData: ResumeData): Promise<void> => {
  ...
  try {
    ...
    const canvas = await html2canvas(element, {
      scale: 2.0, // 2x scale for crisp rendering
      useCORS: true,
      allowTaint: false, // Must be false to prevent canvas tainting from external cross-origin assets
    ...
    // Generate JPEG image data URL with 0.98 quality to avoid PNG encoding chunk/signature issues
    let imgData = '';
    ...
    try {
      ...
      console.warn('JPEG dataURL failed, falling back to PNG:', e);
      imgData = canvas.toDataURL('image/png', 1.0);
      imgFormat = 'PNG';
    }
